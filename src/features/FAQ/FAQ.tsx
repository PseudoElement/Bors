...
const FAQ = () => {
  ...
  return (
      <div className={s.container}>
        ...
        <Tabs selectedIndex={activeTabIndex} onSelect={handleTabSelect} className={s.faqBlock}>
           <div className={s.faqTabList}>
             <TabList>
               {mock__faq.map((tab, index) => (
                 <Tab key={tab.id} className={cn(index === activeTabIndex ? s.active : '', s.tabList)}>{tab.title}</Tab>
              ))}
            </TabList>
            </div>
            <div  className={s.faqTabPanel}>
            {mock__faq.map((tab) => (
              <TabPanel key={tab.id} className={s.tabPanel}>{tab.text}</TabPanel>
            ))}
          </div>
        </Tabs>
